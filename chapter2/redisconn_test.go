...
func TestLoginCookies(t *testing.T) {
	...
	t.Run("Test UpdateToken", func(t *testing.T) {
		...
		s, _ := redis.Int64(conn.Do("HLEN", "login:"))
	...
	t.Run("Test shopping cart cookie", func(t *testing.T) {
		...
		r, _ := redis.StringMap(conn.Do("HGETALL", "cart:"+token))
	...
	t.Run("Test cache request", func(t *testing.T) {
		client.UpdateTokenModified(token, "username", "itemX")
	...
	t.Run("Test cache row", func(t *testing.T) {
		...
		s, _ := redis.IntMap(client.Conn.Do("ZRANGE", "schedule:", 0, -1, "WITHSCORES"))
		t.Log(s)
		...
		r, _ := redis.String(client.Conn.Do("GET", "inv:itemX"))
		t.Log(r)
		...
		r2, _ := redis.String(client.Conn.Do("GET", "inv:itemX"))
		t.Log(r2)
		utils.AssertTrue(t, len(r) != 0)
